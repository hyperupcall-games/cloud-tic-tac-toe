...
func _ready():
	get_node('TurnStatus').text = player1Name + "'s turn"
...
func show_status():
	...
	if is_solved():
		...
		var congrats_name = player1Name
		if currentTurn == 'player1':
			congrats_name = player1Name

		get_node('SolveStatus').text = 'SOLVED!'
		get_node('GoodJobStatus').text = 'Good job %s :)' % congrats_name
		get_node('TurnStatus').text = ''
		currentTurn = ''
	elif is_cats_game():
		singleton.play_sad()
...
func switchTurn():
	if currentTurn == 'player1':
		currentTurn = 'player2'
		get_node('TurnStatus').text = player2Name + "'s turn"
